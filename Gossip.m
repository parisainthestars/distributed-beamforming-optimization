...
hist_power = [];
hist_cvx = [];
fprintf('Starting Asynchronous Gossip Distributed Projected Gradient Descent...\n');

for ccp = 1:max_ccp_iter
    W_tensor = cat(3, W_nodes{:});
    W_avg_global = mean(W_tensor, 3);
    W_prev_mat = W_avg_global;
    
    for k = 1:max_gd_iter
        alpha_k = 10 / ((1 +  k)*(ccp));
        
        pair_idx = randperm(N, 2);
        idx_A = pair_idx(1);
        idx_B = pair_idx(2);
        
        W_mix = 0.5 * (W_nodes{idx_A} + W_nodes{idx_B});
        
        W_nodes{idx_A} = update_node(W_mix, W_prev_mat, idx_A, alpha_k, H, gamma_vec, P_vec, sigma2, rho_penalty, N, M, K);
        W_nodes{idx_B} = update_node(W_mix, W_prev_mat, idx_B, alpha_k, H, gamma_vec, P_vec, sigma2, rho_penalty, N, M, K);
    end
    ...
    cvx_begin quiet
        ...
        subject to
            ...
            for k_user = 1:K
                ...
                h_wk_prev = H(:,k_user)' * W_prev_mat(:,k_user);
    ...
    fprintf('CCP %2d: Gossip Power = %.4f | CVX Power = %.4f\n', ccp, curr_power, hist_cvx(end));
end

figure;
semilogy(hist_power, '-s', 'LineWidth', 2, 'DisplayName', 'Gossip DPGD');
hold on;
semilogy(hist_cvx, '-o', 'LineWidth', 2, 'DisplayName', 'Centralized CVX');
grid on; legend;
xlabel('CCP Iteration'); ylabel('Total Power (Watts)');
title('Convergence of Gossip-Based Algorithm');
...
for n=1:N
    ...
    if per_antenna_power(n) <= P_vec(n) + 1e-4, fprintf(' [OK]\n'); else, fprintf(' [VIOLATED]\n'); end
end
...
for k=1:K
    ...
    if sinr_vals(k) >= gamma_vec(k) * 0.95, fprintf('[OK]\n'); else, fprintf('[VIOLATED]\n'); sat_sinr = false; end
end
...
function W_new = update_node(W_current, W_prev_mat, i_node, alpha, H, gamma_vec, P_vec, sigma2, rho_penalty, N, M, K)
    Grad_Total = 2 * W_current;
    
    for u = 1:K
        interf_power = sigma2;
        for j = 1:M
            if j ~= u
                interf_power = interf_power + abs(H(:,u)' * W_current(:,j))^2;
            end
        end
        
        h_prev = H(:,u)' * W_prev_mat(:,u);
        h_curr = H(:,u)' * W_current(:,u);
        ...
        if constraint_val > 0
            ...
            for j = 1:M
                if j ~= u
                    h_u = H(:,u);
                    Grad_Interf = 2 * h_u * (h_u' * W_current(:,j));
    ...
    W_temp = W_current - alpha * Grad_Total;
